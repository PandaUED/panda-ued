...
(function(window) {
  ...
  SMApp.fn = SMApp.prototype = {
    ...
    percentageSize(size, size2) {
      return `${Math.round(size / size2 * 1000) / 10}%`;
    },
    unitSize(length, isText) {
    ...
    isIntersect(selectedRect, targetRect) {
      return !(
    ...
    getRect(index) {
      const layer = this.current.layers[index];
      return {
        x: layer.rect.x,
        y: layer.rect.y,
    ...
    message(msg) {
      const $message = $('#message').text(msg);
      $message
        .hide()
        .fadeIn()
        .delay(1000)
    ...
    render() {
      $('#app').html(
        [
          ...
          `<section id="slices" style="display: none;"><div class="empty">${_(
            'No slices added!'
          )}</div></section>`,
          `<section id="colors" style="display: none;"><div class="empty">${_(
            'No colors added!'
          )}</div></section>`,
          '</aside>',
    ...
    screen() {
      const imageData = this.current.imageBase64
        ? this.current.imageBase64
        : this.current.imagePath + timestamp;
      ...
      if (!this.maxSize) {
        let screenSize =
            this.current.width > this.current.height
              ...
              : this.current.height,
          artboardSize =
            $('.screen-viewer').outerWidth() > $('.screen-viewer').outerHeight()
              ? $('.screen-viewer').outerWidth()
              : $('.screen-viewer').outerHeight();
        this.maxSize =
          screenSize > artboardSize ? screenSize * 5 : artboardSize * 5;
        ...
        let scrollMaxX = this.maxSize - $('.screen-viewer').outerWidth(),
          scrollMaxY = this.maxSize - $('.screen-viewer').outerHeight(),
          scrollLeft = 0.5 * scrollMaxX,
          scrollTop = 0.5 * scrollMaxY;
      ...
      $('#screen').css({
        ...
        backgroundSize: `${this.zoomSize(this.current.width)}px ${this.zoomSize(
          this.current.height
        )}px`,
      });
    ...
    zoom() {
      let zoomText = `${this.configs.zoom * 100}%`,
        inDisabled = this.configs.zoom <= 0.25 ? ' disabled="disabled"' : '',
    ...
    unit() {
      let self = this,
        ...
        unitCurrent = '',
        hasCurrent = '';
      $.each(unitsData, (index, data) => {
        if (data.name)
          unitList.push(`<li class="sub-title">${_(data.name)}</li>`);
        $.each(data.units, (index, unit) => {
          let checked = '';
          // if(unit.scale === self.configs.scale){
          if (
            unit.unit === self.configs.unit &&
            unit.scale === self.configs.scale
          ) {
            checked = ' checked="checked"';
            hasCurrent = _(unit.name);
          }
          unitList.push(
            `<li><label><input type="radio" name="resolution" data-name="${_(
              unit.name
            )}" data-unit="${unit.unit}" data-scale="${unit.scale}"${checked}><span>${_(
              unit.name
            )}</span></label></li>`
          );
          // }
      ...
      if (!hasCurrent) {
        unitCurrent = `<li><label><input type="radio" name="resolution" data-name="${_(
          'Custom'
        )} (${self.configs.scale}, ${self.configs.unit})" data-unit="${self
          .configs.unit}" data-scale="${self.configs
          .scale}" checked="checked"><span>${_('Custom')} (${self.configs
          .scale}, ${self.configs.unit})</span></label></li>`;
        hasCurrent = `${_('Custom')} (${self.configs.scale}, ${self.configs
          .unit})`;
      }
      unitHtml.push(
        '<div class="overlay"></div>',
    ...
    artboards() {
      ...
      $.each(this.project.artboards, (index, artboard) => {
        ...
        let classNames =
            self.artboardIndex === index && !artboard.fileName ? ' active' : '',
          fileName = artboard.fileName
            ? ` data-file="${artboard.fileName}"`
            : '',
          imageData = artboard.imageBase64
            ? artboard.imageBase64
            : artboard.imagePath + timestamp;
      ...
      pagesSelect.push(
        `<h3>${_('All artboards')} <em>(${this.project.artboards
          .length})</em></h3>`
      );
      pagesSelect.push('<ul class="page-list">');
      pagesSelect.push(
        `<li><label><input type="radio" name="page" value="all" checked="checked"><span>${_(
          'All artboards'
        )} <em>(${this.project.artboards.length})</em></span></label></li>`
      );
      $.each(pagesData, (objectID, artboard) => {
        pagesSelect.push(
          `<li><label><input type="radio" name="page" value="${artboard.objectID}"><span>${artboard.name} <em>(${artboard.count})</em></span></label></li>`
        );
      });
      pagesSelect.push('</ul>');
      ...
      $('#artboards').html(
        [pagesSelect.join(''), artboardListHTML.join('')].join('')
      );
      return this;
    },
    layers() {
      let self = this,
        layersHTML = [];
      $.each(this.current.layers, (index, layer) => {
        let x = self.zoomSize(layer.rect.x),
        ...
        layersHTML.push(
          [
            `<div id="layer-${index}" class="${classNames.join(
              ' '
            )}" data-index="${index}" percentage-width="${self.percentageSize(
              layer.rect.width,
              self.current.width
            )}" percentage-height="${self.percentageSize(
              layer.rect.height,
              self.current.height
            )}" data-width="${self.unitSize(
              layer.rect.width
            )}" data-height="${self.unitSize(
              layer.rect.height
    ...
    slices() {
      ...
      $.each(this.project.slices, (index, sliceLayer) => {
        if (sliceLayer.exportable.length > 0) {
          ...
          sliceListHTML.push(
            ...
            `<small>${self.unitSize(sliceLayer.rect.width)} × ${self.unitSize(
              sliceLayer.rect.height
            )}</small>`,
            '</div>',
            '</li>'
          );
    ...
    colors(colors) {
      ...
      $.each(this.project.colors, (index, color) => {
        self.project.colorNames[color.color['argb-hex']] = color.name;
        colorListHTML.push(
          `<li id="color-${index}" data-color="${encodeURI(
    ...
    notes() {
      let self = this,
        notesHTML = [];
      $.each(this.current.notes, (index, note) => {
        notesHTML.push(
          `<div class="note" data-index="${index +
            1}" style="left: ${self.zoomSize(
            note.rect.x
          )}px; top: ${self.zoomSize(
            note.rect.y
          )}px;"><div style="white-space:nowrap;display:none;">${note.note}</div></div>`
        );
      });
    ...
    getEdgeRect(event) {
      ...
      let x = (event.pageX - offset.left) / this.configs.zoom,
    ...
    distance() {
      if (
        ...
        let selectedRect = this.getRect(this.selectedIndex),
        ...
        if (!this.isIntersect(selectedRect, targetRect)) {
          if (selectedRect.y > targetRect.maxY) {
            // top
            topData = {
              x,
          ...
          if (selectedRect.maxX < targetRect.x) {
            // right
            rightData = {
              ...
              percentageDistance: this.percentageSize(
                targetRect.x - selectedRect.maxX,
                this.current.width
              ),
            };
          }
          if (selectedRect.maxY < targetRect.y) {
            // bottom
            bottomData = {
              x,
              ...
              percentageDistance: this.percentageSize(
                targetRect.y - selectedRect.maxY,
                this.current.height
              ),
            };
          }
          if (selectedRect.x > targetRect.maxX) {
            // left
            leftData = {
          ...
          const distance = this.getDistance(selectedRect, targetRect);
          if (distance.top != 0) {
            // top
            topData = {
              x,
    ...
    inspector() {
      ...
      let position = [
          ...
          `<label data-label="${_(
            'X'
          )}"><input id="position-x" type="text" value="${this.unitSize(
            layerData.rect.x
          )}" readonly="readonly"></label>`,
          `<label data-label="${_(
            'Y'
          )}"><input id="position-y" type="text" value="${this.unitSize(
            layerData.rect.y
          )}" readonly="readonly"></label>`,
          '</div>',
        ].join(''),
        size = [
          ...
          `<label data-label="${_(
            'Width'
          )}"><input id="size-width" type="text" value="${this.unitSize(
            layerData.rect.width
          )}" readonly="readonly"></label>`,
          `<label data-label="${_(
            'Height'
          )}"><input id="size-height" type="text" value="${this.unitSize(
            layerData.rect.height
          )}" readonly="readonly"></label>`,
          '</div>',
        ].join(''),
        opacity =
          typeof layerData.opacity === 'number'
            ? [
                `<div class="item" data-label="${_('Opacity')}:">`,
                `<label><input id="opacity" type="text" value="${Math.round(
                  layerData.opacity * 10000
                ) / 100}%" readonly="readonly"></label>`,
                '<label></label>',
                '</div>',
              ].join('')
            : '',
        radius = layerData.radius
          ? [
              ...
              `<label><input id="radius" type="text" value="${this.unitSize(
                layerData.radius
              )}" readonly="readonly"></label>`,
              '<label></label>',
              '</div>',
      ...
      html.push(
        this.propertyType(
          'PROPERTIES',
          [position, size, opacity, radius, styleName].join('')
        )
      );
      // FILLS
      if (layerData.fills && layerData.fills.length > 0) {
        ...
        $.each(fillsData.reverse(), (index, fill) => {
          fills.push(
            `<div class="item items-group" data-label="${_(
              fill.fillType.firstUpperCase()
            )}:">`
          );
          if (fill.fillType === 'color') {
            ...
            $.each(fill.gradient.colorStops, (index, gradient) => {
      ...
      if (layerData.type === 'text') {
        let fontFamily = [
          ...
          fontSize = layerData.fontSize
            ? [
                `<div class="item" data-label="${_('Size')}:">`,
                `<label><input id="opacity" type="text" value="${this.unitSize(
                  layerData.fontSize,
                  true
                )}" readonly="readonly"></label>`,
                '<label></label>',
          ...
          spacing = [
            ...
            `<label data-label="${_(
              'Character'
            )}"><input id="letter-spacing" type="text" value="${this.unitSize(
              layerData.letterSpacing,
              true
            )}" readonly="readonly"></label>`,
            `<label data-label="${_(
              'Line'
            )}"><input id="line-height" type="text" value="${this.unitSize(
          ...
          content = [
            ...
            `<label data-label="${_(
              'Content'
            )}"><textarea id="content" rows="2" readonly="readonly">${layerData.content}</textarea></label>`,
            '</div>',
          ].join('');
        html.push(
          this.propertyType(
            'TYPEFACE',
            [fontFamily, textColor, fontSize, spacing, content].join('')
          )
        );
      }
      // BORDERS
      if (layerData.borders && layerData.borders.length > 0) {
        ...
        $.each(bordersData.reverse(), (index, border) => {
          borders.push(
            '<div class="items-group">',
            `<h4>${_(`${border.position.firstUpperCase()} Border`)}</h4>`,
            `<div class="item" data-label="${_('Weight')}:">`,
            `<label><input id="font-family" type="text" value="${self.unitSize(
              border.thickness
            )}" readonly="readonly"></label>`,
            '<label></label>',
            '</div>'
          ...
          borders.push(
            `<div class="item" data-label="${_(
              border.fillType.firstUpperCase()
            )}:">`
          );
          if (border.fillType === 'color') {
            ...
            $.each(border.gradient.colorStops, (index, gradient) => {
      ...
      if (layerData.shadows && layerData.shadows.length > 0) {
        ...
        $.each(shadowsData.reverse(), (index, shadow) => {
          shadows.push(
            '<div class="items-group">',
            ...
            `<label data-label="${_(
              'X'
            )}"><input id="offset-x" type="text" value="${self.unitSize(
              shadow.offsetX
            )}" readonly="readonly"></label>`,
            `<label data-label="${_(
              'Y'
            )}"><input id="offset-y" type="text" value="${self.unitSize(
              shadow.offsetY
            )}" readonly="readonly"></label>`,
            '</div>',
      ...
      if (layerData.css && layerData.css.length > 0) {
        ...
        const css = [
          ...
          `<label><textarea id="css" rows="${layerData.css.length +
            1}" readonly="readonly">${layerData.css.join(
            '\r\n'
          )}</textarea></label>`,
          '</div>',
        ].join('');
        ...
        if (layerData.type === 'text') {
          ios.push(
            ...
            `${'<label><textarea id="css" rows="6" readonly="readonly">' +
              ...
              'label.frame = CGRectMake('}${this.scaleSize(
              layerData.rect.x
            )}\, ${this.scaleSize(layerData.rect.y)}\, ${this.scaleSize(
              layerData.rect.width
            )}\, ${this.scaleSize(layerData.rect.height)});\r\n` +
              `label.text = \@"${layerData.content}";\r\n` +
              `label.font = [UIFont fontWithName:\@"${layerData.fontFace}" size:${this.scaleSize(
                layerData.fontSize
              )}];\r\n` +
              `label.textColor = [UIColor colorWithRed:${layerData.color
                .r}/255.0 green:${layerData.color.g}/255.0 blue:${layerData
                .color.b}/255.0 alpha:${layerData.color.a}/1.0];\r\n` +
              '</textarea></label>',
            '</div>'
          );
        } else if (layerData.type === 'shape') {
          ios.push(
            ...
            `${'<label><textarea id="css" rows="6" readonly="readonly">' +
              ...
              'view.frame = CGRectMake('}${this.scaleSize(
              layerData.rect.x
            )}\, ${this.scaleSize(layerData.rect.y)}\, ${this.scaleSize(
              layerData.rect.width
            )}\, ${this.scaleSize(
              layerData.rect.height
            )});\r\n${this.getIOSShapeBackground(
              layerData
            )}</textarea></label>`,
            '</div>'
          );
          ...
          ios.push(
            ...
            `${'<label><textarea id="css" rows="6" readonly="readonly">' +
              ...
              'imageView.frame = CGRectMake('}${this.scaleSize(
              layerData.rect.x
            )}\, ${this.scaleSize(layerData.rect.y)}\, ${this.scaleSize(
              layerData.rect.width
            )}\, ${this.scaleSize(
              layerData.rect.height
            )});\r\n${this.getIOSImageSrc(layerData)}</textarea></label>`,
            '</div>'
          );
        }
        html.push(
          this.propertyType(
            'CODE TEMPLATE',
            [tab, css, android.join(''), ios.join('')].join(''),
            true
          )
        );
      }
      ...
      if (layerData.exportable && layerData.exportable.length > 0) {
        ...
        $.each(layerData.exportable, (index, exportable) => {
          const filePath = path + exportable.path;
          expHTML.push(
            '<li>',
            `<a href="${openId}${filePath}" download="${exportable.path}" data-format="${exportable.format.toUpperCase()}"><img src="${openId}${filePath}" alt="${exportable.path}">${exportable.path.replace(
              'drawable-',
              ''
            )}</a>`,
            '</li>'
    ...
    getAndroidWithHeight(layerData) {
      return (
        `android:layout_width="${this.unitSize(
          layerData.rect.width,
          false
        )}"\r\n` +
        `android:layout_height="${this.unitSize(
          layerData.rect.height,
          false
        )}"\r\n`
      );
    ...
    getAndroidShapeBackground(layerData) {
      ...
      if (
        ...
        typeof layerData.fills === 'undefined' ||
        layerData.fills.length === 0
      )
        return colorCode;
      let f;
      for (f in layerData.fills) {
        if (layerData.fills[f].fillType === 'color') {
          return `android:background="${layerData.fills[f].color[
            'argb-hex'
          ]}"\r\n`;
        }
      }
    ...
    getAndroidImageSrc(layerData) {
      if (
        ...
        typeof layerData.exportable === 'undefined' ||
        layerData.exportable === 0
      )
        return '';
      return `android:src="\@mipmap/${layerData.exportable[0].name}.${layerData
        .exportable[0].format}"\r\n`;
    },
    getIOSShapeBackground(layerData) {
      ...
      for (f in layerData.fills) {
        if (layerData.fills[f].fillType === 'color') {
          return `view.backgroundColor = [UIColor colorWithRed:${layerData
            .fills[f].color.r}/255.0 green:${layerData.fills[f].color
            .g}/255.0 blue:${layerData.fills[f].color.b}/255.0 alpha:${layerData
            .fills[f].color.a}/1.0]\;\r\n`;
        }
      }
      return colorCode;
    ...
    getIOSImageSrc(layerData) {
      if (
        ...
        return '';
      return `imageView.image = [UIImage imageNamed:\@"${layerData.exportable[0]
        .name}.${layerData.exportable[0].format}"];\r\n`;
    },
    renderInspector(html) {
      ...
      $('#inspector')
        ...
        .html(html.join(''));
      $('#inspector')
        .find(`[data-codeType=${self.configs.codeType}]`)
        .addClass('select');
      $('#code-tab')
        .unbind('click')
    ...
    propertyType(title, content, isCode) {
    ...
    colorItem(color) {
      let colorName = this.project.colorNames
        ? this.project.colorNames[color['argb-hex']]
        : '';
      colorName = colorName ? ` data-name="${colorName}"` : '';
      return [
        `<div class="color"${colorName}>`,
        `<label><em><i style="background-color:${color[
          'css-rgba'
        ]};"></i></em></label><input data-color="${encodeURI(
          JSON.stringify(color)
        )}" type="text" value="${color[
          this.configs.colorFormat
        ]}" readonly="readonly">`,
        '</div>',
      ].join('');
    },
    ...
    mouseoverLayer() {
      if (this.targetIndex && this.selectedIndex === this.targetIndex)
        return false;
      const $target = $(`#layer-${this.targetIndex}`);
    ...
    events() {
      ...
      $('body')
        .on({
          click(event) {
            if (!$('.screen-viewer').hasClass('moving-screen')) {
              if (
                $(event.target).hasClass('layer') ||
                $(event.target).hasClass('slice-layer')
              ) {
                const selected = !$(event.target).hasClass('slice-layer')
                  ? event.target
                  : $(
                      `.layer-${$(event.target).attr('data-objectid')}`
                    ).first();
          ...
          mousemove(event) {
            if (!$('.screen-viewer').hasClass('moving-screen')) {
              ...
              self.hideDistance();
              if (
                $(event.target).hasClass('screen-viewer') ||
                $(event.target).hasClass('screen-viewer-inner')
              ) {
        ...
        .on('click', 'header, #inspector, .navbar', event => {
        ...
        .on('dragstart', '.exportable img', function(event) {
          ...
          let left =
              event.originalEvent.pageX - offset.left - jQThis.width() / 2,
      ...
      // zoom
      $('#zoom')
        .on('click', '.zoom-in:not(disabled)', event => {
          self.configs.zoom -= 0.25;
          self.zoomRender();
          event.stopPropagation();
        })
        .on('click', '.zoom-out:not(disabled)', event => {
          self.configs.zoom += 0.25;
          self.zoomRender();
          event.stopPropagation();
        });
      $(window)
        .keydown(event => {
          if (
            ...
            (event.which === 187 || event.which === 189 || event.which === 48)
          ) {
            if (event.which === 187 && self.configs.zoom < 4) {
              $('.zoom-out').click();
            } else if (event.which === 189 && self.configs.zoom > 0.25) {
              $('.zoom-in').click();
        ...
        .mousemove(event => {
          ...
          if (
            ...
            $('.screen-viewer').scrollLeft(
              self.moveData.x - event.clientX + self.moveData.scrollLeft
            );
            $('.screen-viewer').scrollTop(
              self.moveData.y - event.clientY + self.moveData.scrollTop
            );
            event.preventDefault();
          }
        })
        .mousedown(event => {
          ...
          if (
            $('.screen-viewer').hasClass('moving-screen') &&
            ($target.hasClass('cursor') || $target.hasClass('overlay'))
          ) {
            self.moveData = {
              x: event.clientX,
              y: event.clientY,
      ...
      // unit
      $('#unit')
        .on('change', 'input[name=resolution]', () => {
          const $checked = $('input[name=resolution]:checked');
      ...
      $('#artboards')
        ...
        .on('click', '.pages-select', event => {
          event.stopPropagation();
        })
        .on('change', 'input[name=page]', function(event) {
          const pObjectID = $('.page-list input[name=page]:checked').val();
          ...
          if (pObjectID != 'all') {
            $('.artboard-list li').each(function() {
              const pageObjectID = $(this).attr('data-page-id');
      ...
      $('#slices')
        .on('mouseover', 'li', function() {
          const id = $(this).attr('data-objectid');
            ...
            $('.screen-viewer').animate(
              {
                scrollLeft:
                  offsets.left +
                  scrolls.left -
                  (viewerSizes.width - sizes.width) / 2,
                scrollTop:
                  offsets.top +
                  scrolls.top -
                  60 -
                  (viewerSizes.height - sizes.height) / 2,
              },
              150
            );
            $(`.layer-${id}`).click();
      ...
      // color format
      $('#inspector').on('click', '.color label', () => {
        self.configs.colorFormat =
          self.configs.colorFormat === 'color-hex'
            ? 'argb-hex'
            : self.configs.colorFormat === 'argb-hex'
              ? 'css-rgba'
              : self.configs.colorFormat === 'css-rgba'
                ? 'ui-color'
                : 'color-hex';
  ...
  const init = (SMApp.fn.init = function(project) {
    const path = this.locationHash();
    ...
    const proportion = $(document).height() / this.current.height;
    if (proportion >= 0.8) {
      this.configs.zoom = 1;
    } else if (proportion >= 0.7) {
      this.configs.zoom = 0.75;
    } else {
      this.configs.zoom = 0.5;
    }
    this.render();
