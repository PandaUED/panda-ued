...
class Preview extends React.Component {
	...
	render() {
		...
		const decodePath = path.join('/api/data', location.pathname.replace('/preview/', '/library/'), '/');
		...
		if(this.state.fullscreen)Preview = Preview.extend`
		position: fixed;
		width: 100%;
		height: 100%;
		top:0;
		...
		return (
			<div>
				...
				<Button onClick={this.onClick}><Icon type={this.state.fullscreen ? 'shrink' : 'arrows-alt'}/></Button>
				<View>
					<Comment/>
				</View>
			</div>
