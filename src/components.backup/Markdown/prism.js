    ...
    Prism = function () {
	    var e = /\blang(?:uage)?-(\w+)\b/i, t = 0, n = _self.Prism = {
		        ...
		        highlightElement: function (t, a, r) {
			        ...
			        o && (l = (o.className.match(e) || [
				        ,
				        ''
			        ])[1].toLowerCase(), i = n.languages[l]), t.className = t.className.replace(e, '')
			                                                                 .replace(/\s+/g, ' ') + ' language-' + l, o = t.parentNode, /pre/i.test(o.nodeName) && (o.className = o.className.replace(e, '')
			        ...
			        if (n.hooks.run('before-highlight', u), a && _self.Worker) {
				        ...
				        g.onmessage = function (e) {u.highlightedCode = e.data, n.hooks.run('before-insert', u), u.element.innerHTML = u.highlightedCode, r && r.call(u.element), n.hooks.run('after-highlight', u), n.hooks.run('complete', u);}, g.postMessage(JSON.stringify({
					                                                                                                                                                                                                                                                               language      : u.language,
					                                                                                                                                                                                                                                                               code          : u.code,
					                                                                                                                                                                                                                                                               immediateClose: !0
				                                                                                                                                                                                                                                                               }));
					        ...
					        for (var s = 0; s < o.length; ++s) {
						        ...
						        for (var m = 0, y = 0; m < r.length; y += r[m].length, ++m) {
							        ...
							        if (!(v instanceof a)) {
								        ...
								        if (b) {
									        c && (f = b[1].length);
									        var w = b.index + f, b = b[0].slice(f), _ = w + b.length, x = v.slice(0, w), O = v.slice(_),
									            S                                                                          = [m, k];
									        x && S.push(x);
	    ...
	    if (a.stringify = function (e, t, r) {
			    ...
			    var o = Object.keys(l.attributes)
			                  .map(function (e) {return e + '="' + (l.attributes[e] || '').replace(/"/g, '&quot;') + '"';})
			                  .join(' ');
...
Prism.languages.css = {
	...
	'style-attr': {
		...
		inside : {
			...
			}, punctuation: /^\s*=\s*['"]|['"]\s*$/, 'attr-value': {pattern: /.+/i, inside: Prism.languages.css}
		},
		alias  : 'language-css'
	}
...
!function (a) {
	...
	a.languages.asciidoc = {
		...
		replacement: a.languages.asciidoc.replacement,
		entity     : a.languages.asciidoc.entity
	}, a.hooks.add('wrap', function (a) {'entity' === a.type && (a.attributes.title = a.content.replace(/&amp;/, '&'));});
}(Prism);
...
Prism.languages.php = Prism.languages.extend('clike', {
	...
	if ('php' === e.language) {
		for (var a, n = 0; a = e.tokenStack[n]; n++) {
			e.highlightedCode = e.highlightedCode.replace('{{{PHP' + (n + 1) + '}}}', Prism.highlight(a, e.grammar, 'php')
			                                                                               .replace(/\$/g, '$$$$'));
		}
		e.element.innerHTML = e.highlightedCode;
	}
}), Prism.hooks.add('wrap', function (e) {'php' === e.language && 'markup' === e.type && (e.content = e.content.replace(/(\{\{\{PHP[0-9]+\}\}\})/g, '<span class="token php">$1</span>'));}), Prism.languages.insertBefore('php', 'comment', {
	markup: {
		pattern: /<[^?]\/?(.*?)>/,
...
!function (a) {
	...
	a.languages.jsx = a.languages.extend('markup', e), a.languages.jsx.tag.pattern = /<\/?[\w\.:-]+\s*(?:\s+(?:[\w\.:-]+(?:=(?:("|')(\\?[\w\W])*?\1|[^\s'">=]+|(\{[\w\W]*?\})))?|\{\.{3}\w+\})\s*)*\/?>/i, a.languages.jsx.tag.inside['attr-value'].pattern = /=(?!\{)(?:('|")[\w\W]*?(\1)|[^\s>]+)/i, a.languages.insertBefore('inside', 'attr-name', {
		spread: {
			pattern: /\{\.{3}\w+\}/,
			inside : {
				punctuation : /\{|\}|\./,
...
!function (e) {
	...
	}), e.hooks.add('before-highlight', function (e) {'smarty' === e.language && (e.tokenStack = [], e.backupCode = e.code, e.code = e.code.replace(t, function (t) {return t === n && (o = !1), o ? t : (t === a && (o = !0), e.tokenStack.push(t), '___SMARTY' + e.tokenStack.length + '___');}));}), e.hooks.add('before-insert', function (e) {'smarty' === e.language && (e.code = e.backupCode, delete e.backupCode);}), e.hooks.add('after-highlight', function (t) {
		if ('smarty' === t.language) {
			for (var a, n = 0; a = t.tokenStack[n]; n++) {
				t.highlightedCode = t.highlightedCode.replace('___SMARTY' + (n + 1) + '___', e.highlight(a, t.grammar, 'smarty')
				                                                                              .replace(/\$/g, '$$$$'));
			}
	...
	if ('undefined' != typeof self && self.Prism && self.document) {
		...
		var a                 = Prism.plugins.toolbar.registerButton = function (n, a) {
			...
			o = 'function' == typeof a ? a : function (t) {
				var e;
				return 'function' == typeof a.onClick ? (e = document.createElement('button'), e.type = 'button', e.addEventListener('click', function () {a.onClick.call(this, t);})) : 'string' == typeof a.url ? (e = document.createElement('a'), e.href = a.url) : e = document.createElement('span'), e.textContent = a.text, e;
			}, t.push(e[n] = o);
			...
			if (o && /pre/i.test(o.nodeName) && !o.classList.contains('code-toolbar')) {
				...
				r.classList.add('toolbar'), document.body.hasAttribute('data-toolbar-order') && (t = document.body.getAttribute('data-toolbar-order')
				                                                                                             .split(',')
				                                                                                             .map(function (t) {return e[t] || n;})), t.forEach(function (t) {
					var e = t(a);
					if (e) {
		...
		a('label', function (t) {
			...
			if (e && /pre/i.test(e.nodeName) && e.hasAttribute('data-label')) {
				...
				try {a = document.querySelector('template#' + o);} catch (r) {}
...
!function () {
	...
	e.prototype = {
		setDefaults          : function (e) {this.defaults = r(this.defaults, e);},
		...
		spacesToTabs         : function (e, n) {return n = 0 | n || 4, e.replace(new RegExp(' {' + n + '}', 'g'), '	');},
		removeTrailing       : function (e) {return e.replace(/\s*?$/gm, '');},
		removeInitialLineFeed: function (e) {return e.replace(/^(?:\r?\n|\r)/, '');},
		removeIndent         : function (e) {
			var n = e.match(/^[^\S\n\r]*(?=\S)/gm);
			return n && n[0].length ? (n.sort(function (e, n) {return e.length - n.length;}), n[0].length ? e.replace(new RegExp('^' + n[0], 'gm'), '') : e) : e;
		},
...
!function () {
	if ('undefined' != typeof self && self.Prism && self.document) {
		...
		Prism.plugins.toolbar.registerButton("copy-to-clipboard", function (t) {
			function n() {
				var e = new o(i, {text: function () {return t.code}});
				e.on("success", function () {i.textContent = "Copied!", r()}), e.on("error", function () {i.textContent = "Press Ctrl+C to copy", r()})
			}
