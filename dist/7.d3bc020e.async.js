      ...
      (function(e) {
        ...
        (i.fn = i.prototype = {
          ...
          screen: function() {
            ...
            if (!this.maxSize) {
              ...
              var a = this.maxSize - $('.screen-viewer').outerWidth(),
                ...
                o = 0.5 * a,
                r = 0.5 * s;
              $('.screen-viewer').scrollLeft(o),
                $('.screen-viewer').scrollTop(r);
            }
            $('#screen').css({
          ...
          artboards: function() {
            ...
            return (
              ...
              $.each(this.project.artboards, function(t, a) {
                ...
                var o = e.artboardIndex !== t || a.fileName ? '' : ' active',
                  r = a.fileName ? ' data-file="' + a.fileName + '"' : '',
          ...
          layers: function() {
            ...
            $.each(this.current.layers, function(i, a) {
              var s = e.zoomSize(a.rect.x),
                o = e.zoomSize(a.rect.y),
                r = e.zoomSize(a.rect.width),
            ...
            return (
              ...
              $.each(this.project.slices, function(i, a) {
                if (a.exportable.length > 0) {
                  var s = JSON.parse((0, r.default)(a.exportable)).pop();
                  t.push(
                    '<li id="slice-' +
                      a.objectID +
                      ...
                      : a <= 0 && r
                        ...
                        : i >= this.current.width && n
                          ? ((i = this.current.width),
                            (a = 0),
                            (o = this.current.height))
                          : a >= this.current.height && r
                            ? ((i = 0),
                              (a = this.current.height),
                              (s = this.current.width))
                            : i <= 0 &&
                              n &&
                              ((i = -10), (a = 0), (o = this.current.height)),
              r &&
                n &&
                ((i = 0),
                (a = 0),
                (s = this.current.width),
                (o = this.current.height)),
              { x: i, y: a, width: s, height: o, maxX: i + s, maxY: a + o }
            );
          },
          distance: function() {
            if (
              ...
              i &&
                ...
                o &&
                  ($('#ld')
                    .css({ left: o.x, top: o.y, width: o.w })
                    .show(),
                  $('#ld div')
                    .attr('data-width', o.distance)
                    .attr('percentage-width', o.percentageDistance)),
          ...
          inspector: function() {
            ...
            if (
              (a.push(
                this.propertyType('PROPERTIES', [s, o, r, n, l].join(''))
              ),
            ...
            if ('text' === i.type) {
              ...
              a.push(this.propertyType('TYPEFACE', [u, p, m, v, g].join('')));
            }
            if (i.borders && i.borders.length > 0) {
              ...
              $.each(b.reverse(), function(i, a) {
                f.push(
                  ...
                  'color' === a.fillType
                    ? f.push(e.colorItem(a.color))
                    : (f.push('<div class="colors gradient">'),
                      $.each(a.gradient.colorStops, function(t, i) {
                        f.push(e.colorItem(i.color));
                      }),
                      f.push('</div>')),
                  f.push('</div>'),
                  f.push('</div>');
              }),
                a.push(this.propertyType('BORDERS', f.join('')));
            }
                ...
                .on('click', 'li', function() {
                  ...
                  if ($('.layer-' + e).length > 0) {
                    ...
                    $('.screen-viewer').animate(
                      {
                        scrollLeft: a.left + s.left - (r.width - o.width) / 2,
                        scrollTop:
                          a.top + s.top - 60 - (r.height - o.height) / 2,
                      },
                      150
                    ),
                      $('.layer-' + e).click();
